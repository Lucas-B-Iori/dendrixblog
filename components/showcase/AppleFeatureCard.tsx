...
export function AppleFeatureCard() {
  ...
  return (
    <div
      ...
      className="relative rounded-3xl p-5 sm:p-12 overflow-hidden border border-white/10 bg-gradient-to-b from-[#0B1520]/90 via-[#070D14]/95 to-[#04070B] shadow-[0_20px_50px_rgba(0,0,0,0.6)] backdrop-blur-2xl"
    >
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        ...
        <div className="lg:col-span-7 space-y-5">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-mono text-[10px] sm:text-xs font-semibold max-w-full">
            <ShieldCheck className="w-3.5 h-3.5 shrink-0" />
            <span className="truncate">DENDRIX SECURITY CORE • ART. 34 CED-OAB</span>
          </div>
        ...
        <div className="lg:col-span-5">
          <div className="rounded-2xl p-4 sm:p-6 bg-black/40 border border-white/10 backdrop-blur-xl space-y-4">
            ...
            <div className="space-y-3">
              {specs.map((item, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 sm:gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/[0.04] hover:bg-white/[0.05] transition-colors"
                  >
                    ...
                    <span className="text-xs font-mono font-semibold text-emerald-400 pl-9 sm:pl-0 sm:text-right">
                      {item.value}
                    </span>
                  </div>
                );
