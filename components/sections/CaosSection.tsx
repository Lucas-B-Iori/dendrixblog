    ...
    <SectionWrapper id="problema" className="border-t border-slate-200 dark:border-white/[0.08] bg-transparent relative overflow-hidden" spacing="default">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start relative z-10">
        ...
        <div className="lg:col-span-5 space-y-5 text-left">
          <p className="font-mono text-xs font-semibold uppercase tracking-wider text-amber-700 dark:text-amber-400 bg-amber-500/10 border border-amber-500/20 px-3 py-1 rounded-full inline-block">
            O Custo Operacional da Fragmentação
          </p>

          {/* H2 com Leitura Progressiva no Scroll (Skiper 70) */}
          <div className="font-serif text-2xl sm:text-3xl lg:text-4xl font-medium text-slate-900 dark:text-white leading-tight">
            <ScrubbedTextReveal
          ...
          <p className="text-base text-slate-600 dark:text-slate-300 leading-relaxed pt-2">
            A rotina da banca foi fragmentada em abas: o processo no tribunal, o cliente no WhatsApp, os prazos em planilhas e ferramentas genéricas que não conhecem o histórico da sua banca.
          </p>
        </div>
        ...
        <div className="lg:col-span-7">
          <SienaParallax speed={25}>
            <Card3D enableSpotlight={true} spotlightColor="rgba(245, 158, 11, 0.12)">
              <div className="divide-y divide-slate-200 dark:divide-white/[0.08] bg-white dark:bg-[#0A121A]/95 border border-slate-200 dark:border-white/10 rounded-2xl p-3 sm:p-6 shadow-xl dark:shadow-2xl overflow-hidden backdrop-blur-xl">
                {painPoints.map((item, index) => {
                  const Icon = item.icon;
                  return (
                    <div
                      key={index}
                      className="p-4 sm:p-6 group hover:bg-slate-50 dark:hover:bg-white/[0.03] rounded-xl transition-colors"
                    >
                      <div className="flex items-start gap-4">
                        <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5 shadow-xs">
                          <Icon className="w-5 h-5" />
                        </div>
                        <div className="space-y-1.5 flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-mono uppercase tracking-wider text-amber-700 dark:text-amber-400 bg-amber-500/10 px-2.5 py-0.5 rounded border border-amber-500/20 font-medium">
                              {item.tag}
                            </span>
                          </div>
