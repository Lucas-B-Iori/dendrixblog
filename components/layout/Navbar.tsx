  ...
  return (
    <header
      className={`sticky top-0 z-40 w-full transition-all duration-300 ${
        isScrolled
          ? "bg-[#05080C]/90 backdrop-blur-xl border-b border-white/[0.08] shadow-[0_4px_30px_rgba(0,0,0,0.5)]"
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-18">
          ...
          <Link href="/" className="flex items-center gap-2.5 group">
            <span className="font-serif text-2xl sm:text-[1.65rem] font-medium tracking-tight text-white group-hover:text-emerald-400 transition-colors">
              Dendrix
            </span>
            <span className="text-[10px] font-mono uppercase px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-semibold tracking-wider">
              CRM FORENSE
            </span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <a
              ...
              className="inline-flex items-center gap-1 text-sm font-medium text-slate-300 hover:text-white transition-colors px-2.5 py-1.5"
            >
              Entrar
          ...
          <div className="flex md:hidden items-center gap-2">
            ...
            <button
              ...
              className="p-2 text-slate-300 hover:text-white focus-ring rounded-lg border border-white/10 bg-white/[0.03]"
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden border-b border-white/10 bg-[#05080C]/98 backdrop-blur-2xl px-4 pt-2 pb-6 space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
          <nav className="flex flex-col space-y-1.5 pt-2">
            {navLinks.map((link) => (
              <a
                ...
                className="px-3 py-2.5 text-sm font-medium text-slate-300 hover:bg-white/[0.05] rounded-lg transition-colors"
              >
                {link.label}
              </a>
            ))}
          ...
          <div className="pt-4 border-t border-white/10 flex flex-col gap-3">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-1.5 w-full py-2.5 text-sm font-medium text-slate-300 bg-white/[0.04] border border-white/10 rounded-xl"
            >
